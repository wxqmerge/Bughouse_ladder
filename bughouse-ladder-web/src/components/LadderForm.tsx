...
export default function LadderForm({ setShowSettings }: LadderFormProps = {}) {
  ...
  const runTests = () => {
    console.log('Starting runTests function');
    ...
    testFileInput.onchange = (e) => {
      ...
      reader.onload = (e) => {
        ...
        console.log('Starting player parsing loop');
        ...
        console.log('All game results:', allGameResults.length);
        ...
        if (loadedPlayers.length > 0) {
          ...
          console.log('Header line created, length:', headerLine.length);

          let output = headerLine + '\n';
          console.log('Output string initialized');
          ...
          for (let i = 0; i < loadedPlayers.length; i++) {
            ...
            output += '\n';
            console.log(`Exported player ${i + 1}/${loadedPlayers.length}`);
          }

          console.log('File construction complete, creating blob');
      ...
      reader.onload = (e) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          const cols: (string | null)[] = [];
          for (let j = 0; j < parts.length && j < hasTail; j++) {
            console.log(`Col length ${j} value length: ${parts[j]?.length || 0}`);
          ...
          allGameResults.push(gameResults);
          console.log(`Game results for player ${loadedPlayers.length}: ${gameResults.length} rounds`);
        }
